...
export default function UserDashboard({ onNavigate }) {
 ...
 <div className="w-10 h-10 border-4 border-blue-600 border-t-transparent rounded animate-spin" />
 </div>
 );
 }
 ...
 {/* Sidebar */}
 <div className="w-full md:w-64 bg-white border-r border-slate-200 p-6 flex flex-col">
 <div className="mb-10">
 <h1 className="text-2xl font-black text-slate-900">User Portal</h1>
 ...
 className={`w-full flex items-center gap-3 px-4 py-3 rounded font-medium transition-colors ${
 activeTab === item.id 
 ...
 className="w-full flex items-center gap-3 px-4 py-3 rounded font-medium text-red-600 hover:bg-red-50 transition-colors"
 >
 ...
 <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-md p-8 md:p-10 text-white shadow-lg relative overflow-hidden">
 <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded blur-3xl -translate-y-1/2 translate-x-1/3"></div>
 <div className="absolute bottom-0 left-0 w-48 h-48 bg-blue-400/20 rounded blur-2xl translate-y-1/3 -translate-x-1/4"></div>
 ...
 <div className="hidden md:flex items-center justify-center w-20 h-20 bg-white/10 backdrop-blur-md rounded-md border border-white/20">
 ...
 <p className="text-3xl font-black mt-2 text-slate-900">{projects.length}</p>
 </div>
 <div className="bg-white p-6 rounded-md shadow-sm border border-slate-200">
 <h3 className="text-slate-500 font-medium">Pending Payments</h3>
 <p className="text-3xl font-black mt-2 text-slate-900">
 {payments.filter(p => p.status === 'pending').length}
 </p>
 </div>
 <div className="bg-white p-6 rounded-md shadow-sm border border-slate-200">
 <h3 className="text-slate-500 font-medium">Upcoming Schedules</h3>
 <p className="text-3xl font-black mt-2 text-slate-900">
 ...
 {projects.length === 0 ? (
 <div className="bg-white rounded-md p-10 text-center border border-slate-200 text-slate-500">
 You don't have any projects yet.
 </div>
 ) : (
 <div className="grid grid-cols-1 gap-8">
 {projects.map((p) => (
 <div key={p._id} className="bg-white p-6 md:p-8 rounded-md shadow-sm border border-slate-200">
 
 {/* Header: Title & Status */}
 <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
 <div>
 <div className="flex items-center gap-3 mb-2">
 <h3 className="font-bold text-2xl text-slate-900">{p.title}</h3>
 <span className="inline-block px-3 py-1 bg-blue-100 text-blue-800 text-xs rounded uppercase tracking-wide font-bold">
 {p.projectType}
 </span>
 </div>
 {p.description && <p className="text-sm text-slate-600 mt-2 max-w-3xl">{p.description}</p>}
 </div>
 
 <div className="flex flex-col items-end gap-2 shrink-0">
 <span className={`px-4 py-1.5 rounded text-sm font-bold shadow-sm ${
 ...
 <span className="text-lg font-black text-blue-600">{p.completionPercentage || 0}%</span>
 </div>
 <div className="w-full bg-slate-100 rounded h-3">
 <div 
 className="bg-blue-600 h-3 rounded transition-all duration-1000 ease-out relative overflow-hidden" 
 ...
 {/* Meta Data Grid */}
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8 p-5 bg-slate-50 rounded-md">
 <div>
 ...
 <span key={i} className="px-2.5 py-1 bg-white border border-slate-200 rounded-lg text-xs font-semibold text-slate-700">
 {tech}
 </span>
 ))
 ) : (
 ...
 target="_blank" 
 rel="noreferrer"
 className="flex items-center justify-between p-3 rounded border border-slate-200 hover:border-blue-300 hover:bg-blue-50 transition-colors group"
 >
 ...
 <div 
 key={i} 
 className="flex items-center justify-between p-3 rounded border border-slate-200 bg-slate-50 opacity-75 cursor-not-allowed"
 ...
 {file.title} 
 <span className="text-[10px] bg-red-100 text-red-700 px-2 py-0.5 rounded font-bold">LOCKED</span>
 </p>
 ...
 <p className="text-sm text-slate-500 italic p-4 bg-slate-50 rounded border border-slate-100">
 ...
 <div className="space-y-0 relative before:absolute before:inset-0 before:ml-2 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-200 before:to-transparent pl-6 md:pl-0">
 ...
 <div className="flex items-center justify-center w-4 h-4 rounded border-2 border-white bg-indigo-500 absolute left-[-24px] md:left-1/2 md:-translate-x-1/2"></div>
 <div className="w-full md:w-5/12 bg-white p-3 rounded border border-slate-100 shadow-sm">
 <div className="flex justify-between items-center mb-1">
 <span className="font-bold text-sm text-slate-900">{vh.version}</span>
 <span className="text-[10px] font-medium text-slate-500 bg-slate-100 px-2 py-0.5 rounded">
 {new Date(vh.date).toLocaleDateString()}
 </span>
 </div>
 <p className="text-xs text-slate-600">{vh.description}</p>
 </div>
 </div>
 ))
 ) : (
 <p className="text-sm text-slate-500 italic p-4 bg-slate-50 rounded border border-slate-100 relative z-10 w-full text-center">
 ...
 <td className="p-4 text-sm font-medium text-slate-900">₹{p.amount}</td>
 <td className="p-4 text-sm text-slate-600 capitalize">{p.type}</td>
 <td className="p-4">
 ...
 {schedules.map((s) => (
 <div key={s._id} className="bg-white p-6 rounded-md shadow-sm border border-slate-200 relative overflow-hidden group hover:border-blue-300 transition-colors">
 ...
 <div className="mb-4">
 <span className="inline-block px-2.5 py-1 bg-slate-100 text-slate-700 text-[10px] rounded uppercase font-black tracking-widest mb-2">
 {s.type}
 </span>
 <h3 className="font-bold text-lg text-slate-900 leading-tight">{s.title}</h3>
 </div>
