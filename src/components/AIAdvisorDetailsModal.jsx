...
export default function AIAdvisorDetailsModal({ project, onClose, onSelectSimilar }) {
 ...
 (project.securityFeatures || ["Role-based Access Control (RBAC)","End-to-End Encryption (AES-256)","JWT Authentication Tokens","SQL Injection & XSS Protection"
 ]).forEach(s => bulletItem(s,"⚙", [124, 58, 237]));

 // Deployment Plan
 sectionTitle("DEPLOYMENT PLAN", [8, 145, 178]);
 (project.deploymentPlan || ["Phase 1: Local Environment Setup & DB Modeling","Phase 2: Core API & Frontend Integration","Phase 3: Testing & Quality Assurance","Phase 4: Cloud Deployment (AWS / Vercel / Railway)"
 ]).forEach((plan, i) => bulletItem(plan, `${i + 1}.`, [8, 145, 178]));

 // Future Scope
 sectionTitle("FUTURE SCOPE", [217, 119, 6]);
 (project.futureScope || ["Integration with Mobile Applications","AI/ML Predictive Analytics module","Blockchain for immutable audit logs"
 ]).forEach(s => bulletItem(s,"→", [217, 119, 6]));

 // Learning Outcomes
 sectionTitle("LEARNING OUTCOMES", [79, 70, 229]);
 (project.learningOutcomes || ["Mastery in full-stack architecture design",
 `Hands-on experience with ${project.technology[0] ||"modern frameworks"}`,"Understanding of modern deployment workflows","Implementation of industry-standard security practices"
 ...
 <div className="relative w-full max-w-4xl bg-white rounded-md border-2 border-slate-900 shadow-sm hover:shadow-md transition-shadow hover:shadow-md transition-shadow overflow-hidden">

 {/* Close */}
 <button
 onClick={onClose}
 className="absolute top-4 right-4 z-10 p-2 bg-slate-100 rounded border border-slate-200 text-slate-500 hover:text-slate-900 transition-colors cursor-pointer"
 >
 <X className="w-5 h-5" />
 </button>
 ...
 <span className="text-[10px] font-black uppercase px-3 py-1 bg-white border-t-2 border-blue-500 text-slate-900 rounded-lg border-2 border-slate-900">
 {project.category}
 </span>
 <span className="text-[10px] font-black uppercase px-3 py-1 bg-white border-t-2 border-blue-500 text-slate-900 rounded-lg border-2 border-slate-900">
 {project.difficulty}
 ...
 <span className="text-[10px] font-black uppercase px-3 py-1 bg-slate-200 text-slate-900 rounded-lg border-2 border-slate-900">
 {project.duration}
 </span>
 </div>
 ...
 <p className="text-sm text-slate-600 leading-relaxed bg-rose-50 p-3 rounded border border-rose-100">
 {project.problemStatement}
 </p>
 </section>
 <section className="space-y-2">
 <h2 className="text-xs font-black uppercase tracking-widest text-emerald-500">Solution Overview</h2>
 <p className="text-sm text-slate-600 leading-relaxed bg-emerald-50 p-3 rounded border border-emerald-100">
 ...
 <li key={i} className="flex items-start gap-2 text-sm text-slate-600 font-medium bg-slate-50 px-3 py-2 rounded-lg border border-slate-100">
 ...
 <div className="space-y-6">
 <section className="space-y-2">
 <h2 className="text-xs font-black uppercase tracking-widest text-slate-900">Technology Stack</h2>
 <div className="flex flex-wrap gap-2">
 {project.technology.map(t => (
 <span key={t} className="text-xs font-black px-3 py-1.5 bg-slate-900 text-white rounded-lg border border-slate-800">
 {t}
 </span>
 ))}
 </div>
 ...
 <li key={i} className="text-sm text-slate-600 flex items-center gap-2 bg-cyan-50 px-3 py-2 rounded-lg border border-cyan-100">
 <span className="font-black text-cyan-600 text-xs">{i + 1}.</span> {p}
 </li>
 ))}
 </ol>
 </section>
 ...
 className="p-3 bg-white border border-slate-200 hover:border-blue-500 rounded flex flex-col gap-2 text-left transition-all cursor-pointer group"
 >
 ...
 {/* ── CTA Contact ── */}
 <section className="bg-slate-900 p-5 sm:p-7 rounded-md border border-slate-800 relative overflow-hidden">
 ...
 className="w-full py-3 px-5 bg-emerald-500 hover:bg-emerald-400 text-white font-black text-sm rounded flex items-center justify-center gap-2 transition-all cursor-pointer"
 >
 ...
 className="w-full py-3 px-5 bg-blue-600 hover:bg-blue-500 disabled:bg-blue-900 text-white font-black text-sm rounded flex items-center justify-center gap-2 transition-all cursor-pointer"
 >
 {isGeneratingPDF
