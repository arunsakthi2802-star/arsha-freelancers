...
export default function ManagerRequests() {
 ...
 <p className="text-sm text-slate-500 mt-1">Track the status of your requested changes.</p>
 </div>
 <button
 ...
 <h3 className="text-sm font-bold text-slate-950">No requests found</h3>
 <p className="text-xs text-slate-500">You haven't requested any changes yet.</p>
 </div>
 ) : (
 <div className="grid gap-4">
 {requests.map((req) => (
 <div key={req._id} className="bg-white border border-slate-200 rounded-md p-5 shadow-sm">
 ...
 <p className="text-xs font-bold text-slate-700 mb-1">Admin Comment:</p>
 <p className="text-sm text-slate-600">{req.adminComment}</p>
 </div>
 )}
 </div>
 ))}
