...
export default function PendingApprovals() {
 ...
 <div>
 <h2 className="text-xl font-black text-slate-950">Permission Control</h2>
 <p className="text-sm text-slate-500 mt-1">Review and approve changes submitted by Managers.</p>
 </div>
 <button
 ...
 {requests.map((req) => (
 <div key={req._id} className="bg-white border-2 border-slate-200 rounded-md p-6 shadow-sm relative overflow-hidden">
 ...
 Manager <span className="font-bold text-slate-900">{req.requestedBy.email}</span> requested to {req.action} a record in {req.module}.
 </p>
 
 <div className="bg-slate-50 p-4 rounded border border-slate-200 overflow-x-auto text-xs font-mono text-slate-700">
 ...
 className="flex-1 flex items-center justify-center gap-2 px-5 py-2.5 bg-green-600 hover:bg-green-500 text-white font-bold text-xs rounded shadow-sm transition-all disabled:opacity-50 cursor-pointer"
 >
 ...
 disabled={processing === req._id}
 className="flex-1 flex items-center justify-center gap-2 px-5 py-2.5 bg-white hover:bg-red-50 text-red-600 border border-red-200 font-bold text-xs rounded transition-all disabled:opacity-50 cursor-pointer"
 >
 <X className="w-4 h-4" />
 Reject
 </button>
 ...
 <div className="bg-white w-full max-w-md rounded-md shadow-xl overflow-hidden border border-slate-200 p-6">
 ...
 className="w-full p-3 bg-slate-50 border border-slate-200 rounded text-sm focus:ring-2 focus:ring-blue-500 outline-none min-h-[100px] mb-4 text-slate-800"
