...
export default function AdminStories() {
 ...
 <div>
 <h2 className="text-lg font-black text-slate-950">Stories Management</h2>
 <p className="text-xs text-slate-400">{total} total stories</p>
 </div>
 <div className="flex gap-2">
 <button onClick={load} className="p-2.5 bg-white border-2 border-slate-200 rounded text-slate-500 cursor-pointer"><RefreshCw className="w-4 h-4" /></button>
 <button onClick={openCreate} className="flex items-center gap-2 px-4 py-2.5 bg-purple-600 hover:bg-purple-500 text-white text-xs font-black rounded border border-slate-200 shadow-sm hover:shadow-md transition-shadow hover:shadow-md transition-shadow cursor-pointer" id="add-story-btn">
 ...
 <div className="bg-white border border-slate-200 rounded-md p-6 w-full max-w-2xl shadow-sm hover:shadow-md transition-shadow hover:shadow-md transition-shadow">
 <div className="flex items-center justify-between mb-5">
 <h3 className="text-sm font-black text-slate-950">{editId ?"Edit Story" :"Create New Story"}</h3>
 ...
 <input type="text" value={form.title} onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))} placeholder="Story title..." required className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-purple-500 font-medium" />
 </div>
 <div className="grid grid-cols-2 gap-3">
 <div className="space-y-1">
 <label className="text-[10px] font-black uppercase text-slate-400">Category</label>
 <select value={form.category} onChange={(e) => setForm((p) => ({ ...p, category: e.target.value }))} className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded text-xs text-slate-800 focus:outline-none cursor-pointer">
 {CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
 </select>
 </div>
 <div className="space-y-1">
 <label className="text-[10px] font-black uppercase text-slate-400">Status</label>
 <select value={form.status} onChange={(e) => setForm((p) => ({ ...p, status: e.target.value }))} className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded text-xs text-slate-800 focus:outline-none cursor-pointer">
 ...
 <textarea value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} rows={8} placeholder="Write your story here..." required className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-purple-500 resize-vertical font-medium" />
 </div>
 <div className="space-y-1">
 <label className="text-[10px] font-black uppercase text-slate-400">Tags (comma separated)</label>
 <input type="text" value={form.tags} onChange={(e) => setForm((p) => ({ ...p, tags: e.target.value }))} placeholder="e.g. IoT, AI, IEEE" className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded text-xs text-slate-800 focus:outline-none" />
 </div>
 <div className="space-y-1">
 <label className="text-[10px] font-black uppercase text-slate-400">Cover Image</label>
 <div onClick={() => fileRef.current?.click()} className="border-2 border-dashed border-purple-300 rounded p-5 text-center cursor-pointer hover:bg-purple-50 transition-colors">
 ...
 <button type="submit" disabled={submitting} className="px-5 py-2.5 bg-purple-600 hover:bg-purple-500 text-white text-xs font-black rounded border border-slate-200 shadow-sm hover:shadow-md transition-shadow hover:shadow-md transition-shadow cursor-pointer disabled:opacity-50 flex items-center gap-1.5">
 {submitting ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <BookOpen className="w-3.5 h-3.5" />}
 {editId ?"Update Story" :"Create Story"}
 </button>
 <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2.5 bg-slate-100 text-slate-600 text-xs font-black rounded border-2 border-slate-200 cursor-pointer">Cancel</button>
 </div>
 </form>
 </div>
 </div>
 ...
 <input type="text" placeholder="Search stories..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="w-full pl-9 pr-3 py-2.5 bg-white border-2 border-slate-200 rounded text-xs text-slate-800 focus:outline-none focus:border-purple-500" id="stories-search-admin" />
 </div>
 <select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setPage(1); }} className="px-3 py-2.5 bg-white border-2 border-slate-200 rounded text-xs cursor-pointer">
 ...
 <p className="font-black text-slate-950 truncate max-w-[200px]">{s.title}</p>
 ...
 <button onClick={() => openEdit(s)} className="p-1.5 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-200 cursor-pointer" title="Edit" id={`edit-story-${s._id}`}><Edit2 className="w-3.5 h-3.5" /></button>
