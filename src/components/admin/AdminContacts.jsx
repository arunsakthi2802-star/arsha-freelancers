...
export default function AdminContacts() {
 ...
 <div>
 <h2 className="text-lg font-black text-slate-950">Contact Enquiries</h2>
 ...
 <h3 className="text-sm font-black text-slate-950">Enquiry Details</h3>
 <span className={`text-[10px] font-black px-2 py-1 rounded-lg ${STATUS_COLORS[viewItem.status]}`}>{viewItem.status}</span>
 </div>
 <div className="space-y-3 text-xs">
 <div className="grid grid-cols-2 gap-3">
 <div><p className="text-slate-400 mb-0.5">From</p><p className="font-black text-slate-950">{viewItem.name}</p></div>
 <div><p className="text-slate-400 mb-0.5">Date</p><p className="font-bold text-slate-600">{new Date(viewItem.createdAt).toLocaleDateString("en-IN")}</p></div>
 </div>
 ...
 <p className="font-black text-slate-950">{viewItem.subject}</p>
 </div>
 <div className="bg-slate-50 rounded p-3 border border-slate-200">
 <p className="text-slate-400 text-[10px] font-black uppercase mb-1">Message</p>
 <p className="text-slate-700 leading-relaxed">{viewItem.message}</p>
 </div>
 </div>
 <div className="flex gap-2 mt-5">
 <a href={`mailto:${viewItem.email}?subject=Re: ${viewItem.subject}`} className="px-4 py-2 bg-blue-600 text-white text-xs font-black rounded border border-slate-200 cursor-pointer flex items-center gap-1.5">
 <Mail className="w-3.5 h-3.5" /> Reply
 </a>
 <button onClick={() => { handleStatus(viewItem._id,"replied"); setViewItem(null); }} className="px-4 py-2 bg-emerald-100 text-emerald-700 text-xs font-black rounded border border-emerald-200 cursor-pointer">Mark Replied</button>
 <button onClick={() => setViewItem(null)} className="px-4 py-2 bg-slate-100 text-slate-600 text-xs font-black rounded border-2 border-slate-200 cursor-pointer ml-auto">Close</button>
 </div>
 </div>
 </div>
 )}
 ...
 <input type="text" placeholder="Search enquiries..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="w-full pl-9 pr-3 py-2.5 bg-white border-2 border-slate-200 rounded text-xs text-slate-800 focus:outline-none focus:border-red-400" id="contacts-search" />
 </div>
 ...
 <tbody>
 {contacts.map((c) => (
 <tr key={c._id} className={`border-b border-slate-100 hover:bg-slate-50 ${c.status ==="unread" ?"bg-red-50/30" :""}`}>
