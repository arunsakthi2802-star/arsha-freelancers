...
export default function AdminUsers() {
  ...
  const handle = async (id, action) => {
    try {
      ...
      if (action === "admin") {
        const adminPass = prompt("Please enter the administrator password to authorize making this user an Admin:");
        if (adminPass === null) return; // user clicked Cancel
        if (!adminPass) return alert("Administrator password is required!");
        setActionLoading(id + action);
        await changeUserRole(id, "admin", adminPass);
      }
      if (action === "user") { setActionLoading(id + action); await changeUserRole(id, "user"); }
      load();
                  ...
                  <tr key={u._id} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-white text-[10px] font-black flex-shrink-0 ${u.role === "admin" ? "bg-blue-600" : "bg-slate-400"}`}>
                          {u.fullName?.charAt(0) || "?"}
                        </div>
                        <div>
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 rounded-lg font-black text-[10px] ${u.role === "admin" ? "bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-400" : "bg-slate-100 text-slate-600"}`}>{u.role}</span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      {u._id !== me?._id && (
                        <div className="flex items-center gap-1">
                          <button onClick={() => handle(u._id, u.status === "active" ? "block" : "block")} disabled={!!actionLoading} title={u.status === "active" ? "Block" : "Unblock"} className={`p-1.5 rounded-lg cursor-pointer ${u.status === "active" ? "bg-amber-100 text-amber-600 hover:bg-amber-200" : "bg-emerald-100 text-emerald-600 hover:bg-emerald-200"}`} id={`block-user-${u._id}`}>
                          ...
                          <button onClick={() => handle(u._id, u.role === "admin" ? "user" : "admin")} disabled={!!actionLoading} title={u.role === "admin" ? "Make User" : "Make Admin"} className="p-1.5 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-200 cursor-pointer" id={`role-user-${u._id}`}>
                            {u.role === "admin" ? <User className="w-3 h-3" /> : <Shield className="w-3 h-3" />}
                          </button>
