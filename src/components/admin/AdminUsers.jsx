...
export default function AdminUsers() {
  ...
  return (
    <div className="space-y-6">
      ...
      {deleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
          <div className="bg-white dark:bg-slate-900 border-2 border-slate-950 rounded-2xl p-6 w-full max-w-sm shadow-[4px_4px_0px_0px_rgba(220,38,38,0.8)]">
            <h3 className="text-sm font-black text-red-600 dark:text-red-400 mb-1">Danger Zone: Delete User</h3>
            <p className="text-xs text-slate-400 mb-4">You are about to permanently delete <span className="font-bold text-slate-600 dark:text-slate-300">{deleteModal.fullName}</span>.</p>
            <form onSubmit={handleDeleteSubmit} className="space-y-4">
              <div className="space-y-1">
                ...
                <input 
                  ...
                  required 
                  className="w-full px-3 py-2 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-900/50 rounded-xl text-xs text-slate-800 dark:text-white focus:outline-none focus:ring-1 focus:ring-red-500" 
                />
              </div>

              <div className="flex gap-2 pt-2">
                <button type="submit" disabled={deleteLoading} className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white text-xs font-black rounded-xl border-2 border-slate-950 cursor-pointer disabled:opacity-50 flex items-center gap-1.5">
                  {deleteLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Trash2 className="w-3.5 h-3.5" />} Delete Permanently
                </button>
                <button type="button" onClick={() => { setDeleteModal(null); setDeletePass(""); }} className="px-4 py-2 bg-slate-100 text-slate-600 text-xs font-black rounded-xl border-2 border-slate-200 cursor-pointer">Cancel</button>
              </div>
            </form>
          </div>
        </div>
