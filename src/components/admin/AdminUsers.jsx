...
export default function AdminUsers() {
 ...
 e.preventDefault();
 if (!deletePass) return triggerToast("Admin password is required to delete a user.","error");
 
 setDeleteLoading(true);
 try {
 await deleteUser(deleteModal._id, deletePass);
 ...
 {toast && (
 <div className={`fixed top-5 right-5 z-[9999] text-white border border-slate-200 font-black text-xs px-4 py-3.5 rounded shadow-sm hover:shadow-md transition-shadow hover:shadow-md transition-shadow flex items-center gap-2 animate-bounce ${
 ...
 <button onClick={load} className="p-2.5 bg-white border-2 border-slate-200 rounded text-slate-500 cursor-pointer self-start sm:self-auto"><RefreshCw className="w-4 h-4" /></button>
 {me?.role ==="admin" && (
 <button onClick={() => setShowAddForm(true)} className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-500 text-white text-xs font-black rounded border border-slate-200 shadow-sm hover:shadow-md transition-shadow hover:shadow-md transition-shadow cursor-pointer" id="admin-create-user-btn">
 ...
 <div className="bg-white border border-slate-200 rounded-md p-6 w-full max-w-sm shadow-sm hover:shadow-md transition-shadow hover:shadow-md transition-shadow">
 <h3 className="text-sm font-black text-slate-950 mb-1">Reset Password</h3>
 <p className="text-xs text-slate-400 mb-4">For: <span className="font-bold text-slate-600">{resetModal.fullName}</span></p>
 <form onSubmit={handleReset} className="space-y-3">
 <input type="password" value={newPw} onChange={(e) => setNewPw(e.target.value)} placeholder="New password (min 6 chars)" minLength={6} required className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded text-sm text-slate-800 focus:outline-none" />
 <div className="flex gap-2">
 <button type="submit" disabled={resetLoading} className="px-4 py-2 bg-blue-600 text-white text-xs font-black rounded border border-slate-200 cursor-pointer disabled:opacity-50 flex items-center gap-1.5">
 {resetLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Key className="w-3.5 h-3.5" />} Reset
 </button>
 <button type="button" onClick={() => { setResetModal(null); setNewPw(""); }} className="px-4 py-2 bg-slate-100 text-slate-600 text-xs font-black rounded border-2 border-slate-200 cursor-pointer">Cancel</button>
 </div>
 </form>
 </div>
 </div>
 ...
 <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
 <div className="bg-white border border-slate-200 rounded-md p-6 w-full max-w-sm shadow-sm hover:shadow-md transition-shadow hover:shadow-md transition-shadow">
 <h3 className="text-sm font-black text-slate-950 mb-1">Change User Role</h3>
 <p className="text-xs text-slate-400 mb-4">For: <span className="font-bold text-slate-600">{roleModal.fullName}</span></p>
 ...
 className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded text-xs text-slate-800 focus:outline-none cursor-pointer"
 >
 ...
 <button type="submit" disabled={roleLoading} className="px-4 py-2 bg-amber-500 hover:bg-amber-400 text-white text-xs font-black rounded border border-slate-200 cursor-pointer disabled:opacity-50 flex items-center gap-1.5">
 {roleLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Shield className="w-3.5 h-3.5" />} Update Role
 </button>
 <button type="button" onClick={() => { setRoleModal(null); setAdminPass(""); }} className="px-4 py-2 bg-slate-100 text-slate-600 text-xs font-black rounded border-2 border-slate-200 cursor-pointer">Cancel</button>
 </div>
 </form>
 </div>
 </div>
 ...
 <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
 <div className="bg-white border border-slate-200 rounded-md p-6 w-full max-w-sm shadow-sm hover:shadow-md transition-shadow hover:shadow-md transition-shadow">
 <h3 className="text-sm font-black text-red-600 mb-1">Danger Zone: Delete User</h3>
 <p className="text-xs text-slate-400 mb-4">You are about to permanently delete <span className="font-bold text-slate-600">{deleteModal.fullName}</span>.</p>
 ...
 required 
 className="w-full px-3 py-2 bg-red-50 border border-red-200 rounded text-xs text-slate-800 focus:outline-none focus:ring-1 focus:ring-red-500" 
 />
 </div>

 <div className="flex gap-2 pt-2">
 <button type="submit" disabled={deleteLoading} className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white text-xs font-black rounded border border-slate-200 cursor-pointer disabled:opacity-50 flex items-center gap-1.5">
 {deleteLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Trash2 className="w-3.5 h-3.5" />} Delete Permanently
 </button>
 <button type="button" onClick={() => { setDeleteModal(null); setDeletePass(""); }} className="px-4 py-2 bg-slate-100 text-slate-600 text-xs font-black rounded border-2 border-slate-200 cursor-pointer">Cancel</button>
 </div>
 </form>
 </div>
 </div>
 ...
 <div className="bg-white border border-slate-200 rounded-md p-6 w-full max-w-2xl shadow-sm hover:shadow-md transition-shadow hover:shadow-md transition-shadow my-8">
 <div className="flex items-center justify-between mb-4">
 <h3 className="text-sm font-black text-slate-950">Manage Project Resources & Assets</h3>
 <button onClick={() => setResourceModal(null)} className="text-slate-400 hover:text-slate-950"><X className="w-5 h-5" /></button>
 </div>
 
 <p className="text-xs text-slate-400 mb-4">Client: <span className="font-bold text-slate-600">{resourceModal.fullName}</span></p>
 ...
 className="w-1/2 px-2 py-2 bg-slate-50 border border-slate-200 rounded text-xs text-slate-800 focus:outline-none cursor-pointer"
 >
 ...
 className="w-1/2 px-3 py-2 bg-slate-50 border border-slate-200 rounded text-xs text-slate-800 focus:outline-none"
 />
 </div>
 </div>
 <div className="space-y-1">
 ...
 className="w-full pl-8 pr-3 py-2 bg-slate-50 border border-slate-200 rounded text-xs text-slate-800 focus:outline-none"
 />
 </div>
 </div>
 </div>
 ...
 <div className="border-2 border-dashed border-slate-200 rounded p-4 space-y-3">
 <h4 className="text-xs font-black text-slate-900 uppercase tracking-wider">Add Custom Resource (PDF, ZIP, Image, Doc)</h4>
 ...
 className="w-full px-2.5 py-2 bg-slate-50 border border-slate-200 rounded text-xs text-slate-800 focus:outline-none cursor-pointer"
 >
 ...
 <div className="sm:col-span-2 flex items-center justify-center gap-1 bg-slate-50 px-2 rounded border border-slate-200">
 <input
 type="checkbox"
 ...
 <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 mt-1 bg-slate-50 p-2.5 rounded border border-slate-200">
 <span className="text-[10px] font-black uppercase text-slate-400">Or Upload local file:</span>
 <div className="flex items-center gap-2">
 <input
 type="file"
 onChange={(e) => handleFileUpload(e,"new")}
 className="text-xs text-slate-500 file:mr-3 file:py-1 file:px-2.5 file:rounded-md file:border-0 file:text-[10px] file:font-black file:bg-slate-250 file:text-slate-700 file:cursor-pointer"
 ...
 className="px-3.5 py-1.5 bg-slate-900 text-white text-xs font-black rounded-lg border border-slate-200 shadow-sm hover:shadow-md transition-shadow hover:shadow-md transition-shadow hover:translate-y-[-1px] transition-all cursor-pointer"
 >
 + Add Asset
 </button>
 </div>
 ...
 {resources.length === 0 ? (
 <p className="text-center py-4 bg-slate-50 rounded text-[11px] text-slate-400 font-medium">No custom assets shared yet. Use form above to add report PDFs, code ZIPs, etc.</p>
 ) : (
 <div className="max-h-56 overflow-y-auto space-y-2 border border-slate-100 rounded p-2 bg-slate-50/50">
 ...
 className="w-full px-1 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs cursor-pointer"
 >
 <option value="drive">Drive</option>
 ...
 <div className="flex items-center gap-2 mt-1 bg-slate-50 p-1.5 rounded-lg border border-slate-250">
 <span className="text-[10px] font-black text-slate-400">Or Upload local file:</span>
 <input
 type="file"
 onChange={(e) => handleFileUpload(e, idx)}
 className="text-[10px] text-slate-500 file:mr-2 file:py-0.5 file:px-2 file:rounded file:border-0 file:text-[9px] file:font-black file:bg-slate-200 file:text-slate-700 file:cursor-pointer"
 ...
 <span className="text-[10px] font-black uppercase px-2 py-0.5 bg-slate-100 rounded text-slate-500">{item.fileType}</span>
 ...
 <div className="flex gap-2 pt-2">
 <button type="submit" disabled={resourceLoading} className="px-4 py-2.5 bg-teal-600 text-white text-xs font-black rounded border border-slate-200 cursor-pointer disabled:opacity-50 flex items-center gap-1.5">
 {resourceLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <FolderOpen className="w-3.5 h-3.5" />} Save Shared Resources
 </button>
 <button type="button" onClick={() => setResourceModal(null)} className="px-4 py-2.5 bg-slate-100 text-slate-600 text-xs font-black rounded border-2 border-slate-200 cursor-pointer">Cancel</button>
 </div>
 </form>
 </div>
 </div>
 ...
 {showAddForm && (
 <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
 <div className="bg-white border border-slate-200 rounded-md p-6 w-full max-w-lg shadow-sm hover:shadow-md transition-shadow hover:shadow-md transition-shadow">
 <div className="flex items-center justify-between mb-4">
 <h3 className="text-sm font-black text-slate-950">Create New User</h3>
 ...
 <button type="submit" disabled={addUserLoading} className="px-4 py-2 bg-blue-600 text-white text-xs font-black rounded border border-slate-200 cursor-pointer disabled:opacity-50 flex items-center gap-1.5">
 {addUserLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Plus className="w-3.5 h-3.5" />} Create User
 </button>
 <button type="button" onClick={() => setShowAddForm(false)} className="px-4 py-2 bg-slate-100 text-slate-600 text-xs font-black rounded border-2 border-slate-200 cursor-pointer">Cancel</button>
 </div>
 </form>
 </div>
 </div>
 ...
 <input type="text" placeholder="Search by name or email..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="w-full pl-9 pr-3 py-2.5 bg-white border-2 border-slate-200 rounded text-xs text-slate-800 focus:outline-none focus:border-blue-500" id="users-search" />
 </div>
 ...
 u.role ==="manager" ?"bg-amber-500" :"bg-slate-400"
 }`}>
 {u.fullName?.charAt(0) ||"?"}
 </div>
 <div>
 <p className="font-black text-slate-950">{u.fullName}</p>
 <p className="text-slate-400">{u.college ||"—"}</p>
 <div className="mt-1 flex flex-wrap gap-1">
 {u.resources && u.resources.length > 0 ? (
 <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-teal-50 text-teal-700 text-[9px] font-black border border-teal-200">
 📦 {u.resources.length} asset{u.resources.length > 1 ?"s" :""} ({u.resources.filter(r => r.isUnlocked).length} open)
 </span>
 ) : (
 <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-slate-50 text-slate-400 text-[9px] font-black border border-slate-200">
 📁 No assets
 </span>
 )}
 </div>
 ...
 <td className="px-4 py-3 text-slate-600 font-mono text-[10px] bg-slate-50/50 rounded-lg">{u.uniqueId ||"—"}</td>
 <td className="px-4 py-3 text-slate-600 max-w-[140px] truncate">{u.email}</td>
 ...
 <button disabled={page === 1} onClick={() => setPage((p) => p - 1)} className="px-3 py-2 text-xs font-bold bg-white border-2 border-slate-200 rounded disabled:opacity-40 cursor-pointer">← Prev</button>
 <span className="px-3 py-2 text-xs font-bold text-slate-500">Page {page} / {Math.ceil(total / 15)}</span>
 <button disabled={page >= Math.ceil(total / 15)} onClick={() => setPage((p) => p + 1)} className="px-3 py-2 text-xs font-bold bg-white border-2 border-slate-200 rounded disabled:opacity-40 cursor-pointer">Next →</button>
 </div>
 )}
 </div>
 );
