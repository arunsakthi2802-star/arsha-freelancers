...
const STAR_TAGS = {
 ...
 4: ["Great Support","On-Time Delivery","Professional","Highly Detailed Docs",
 ],
 5: ["Excellent Work","Super Fast Delivery","Top Quality Code","100% Recommended","Viva Ready","Beyond Expectations",
 ],
};

const SERVICE_OPTIONS = ["Final Year Academic Project","Mini Semester Project","IEEE Report Documentation","Documentation and Report","Seminar / Presentation PPT","Python / AI / ML Project","Web / Mobile App Development","ATS Resume & Portfolio","Cybersecurity Project","IoT / Embedded Project","Other Service",
];
...
function StarRating({ value, onChange, size ="lg" }) {
 ...
 isActive
 ?"fill-amber-400 text-amber-400 drop-shadow-sm hover:shadow-md transition-shadow hover:shadow-md transition-shadow"
...
function ReviewCard({ review, index }) {
 return (
  ...
  className="bg-transparent border border-slate-200/50 rounded-3xl p-5 flex flex-col gap-3 text-left"
  >
 ...
 {/* Service badge */}
 {review.service && (
 <span className="inline-block text-[9px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded uppercase tracking-wide w-fit">
 {review.service}
 </span>
 )}

 {/* Review text */}
 <p className="text-xs text-slate-600 leading-relaxed font-medium italic flex-grow">
 ...
 <span
 key={tag}
 className="px-2 py-0.5 bg-amber-50 text-amber-700 text-[9px] font-bold rounded-lg border border-amber-200"
 >
 {tag}
 </span>
 ))}
 ...
 {/* Admin Reply */}
 {review.adminReply && (
 <div className="mt-2 p-3 bg-slate-50/40 backdrop-blur-md border-l-4 border-blue-500 rounded-r-xl relative overflow-hidden group">
 <div className="flex items-center gap-2 mb-1.5">
 <div className="w-5 h-5 rounded bg-blue-600 flex items-center justify-center text-[10px] text-white font-black">A</div>
 <span className="text-[10px] font-black uppercase text-blue-600 tracking-wider">Response from Arsha</span>
 </div>
 ...
 <div
 className={`w-8 h-8 rounded ${review.color} flex items-center justify-center text-white font-black text-xs border border-slate-200 flex-shrink-0`}
 >
 {review.avatar}
 </div>
 <div>
...
export default function ReviewView() {
 ...
 <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-50 border border-amber-200 text-amber-700 text-xs font-black rounded uppercase tracking-wider">
 <Star className="w-3.5 h-3.5 fill-amber-500 text-amber-500" />
 Client Reviews
 </span>
 <h1
 className="text-3xl sm:text-4xl lg:text-5xl font-display font-black leading-tight text-slate-950 tracking-tight"
 ...
 {submitted ? (
 /* ── SUCCESS STATE ── */
 <div className="bg-white border border-slate-200 rounded-md p-8 sm:p-12 text-center space-y-6">
 <div className="flex justify-center">
 <div className="w-20 h-20 bg-emerald-100 text-emerald-600 rounded border-2 border-slate-200 flex items-center justify-center animate-bounce">
 ...
 <button
 onClick={handleReset}
 className="px-6 py-3 bg-slate-900 text-white font-extrabold text-xs rounded border border-slate-200 cursor-pointer"
 >
 ...
 target="_blank"
 rel="noreferrer"
 className="inline-flex items-center justify-center gap-1.5 px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold text-xs rounded border border-slate-200"
 >
 ...
 :"bg-slate-50/40 backdrop-blur-md text-slate-400"
 } ${s.num < 3 ?"border-r-2 border-slate-950" :""}`}
 >
 {step > s.num ?"✓" : `${s.num}. `}
 {s.label}
 </div>
 ...
 <span
 className={`text-sm font-black px-3 py-1 rounded border border-slate-200 ${
 rating === 5
 ...
 :"bg-slate-50/40 backdrop-blur-md text-slate-700 border-slate-200 hover:border-blue-400"
 }`}
 >
 {tag}
 </button>
 ...
 onClick={() => setStep(2)}
 className={`w-full py-3.5 font-extrabold text-sm rounded border border-slate-200 transition-all ${
 canProceedStep1
 ...
 className="w-full px-3.5 py-2.5 bg-slate-50/40 backdrop-blur-md border-2 border-slate-200 rounded text-sm text-slate-800 focus:outline-none focus:border-blue-500 font-bold placeholder:font-normal placeholder:text-slate-400"
 />
 </div>
 ...
 className="w-full px-3.5 py-2.5 bg-slate-50/40 backdrop-blur-md border-2 border-slate-200 rounded text-sm text-slate-800 focus:outline-none focus:border-blue-500 font-bold cursor-pointer"
 >
 ...
 className="w-full px-3.5 py-2.5 bg-slate-50/40 backdrop-blur-md border-2 border-slate-200 rounded text-sm text-slate-800 focus:outline-none focus:border-blue-500 font-medium placeholder:text-slate-400 resize-none"
 />
 </div>
 ...
 type="button"
 onClick={() => setStep(1)}
 className="flex-1 py-3 bg-slate-100 text-slate-700 font-extrabold text-xs rounded border border-slate-200 cursor-pointer hover:bg-slate-200 transition-colors"
 >
 ← Back
 </button>
 <button
 ...
 onClick={() => setStep(3)}
 className={`flex-[2] py-3 font-extrabold text-sm rounded border border-slate-200 transition-all ${
 canProceedStep2
 ?"bg-blue-600 hover:bg-blue-500 text-white cursor-pointer"
 :"bg-slate-100 text-slate-400 cursor-not-allowed"
 }`}
 ...
 {/* Preview card */}
 <div className="bg-slate-50/40 backdrop-blur-md border-2 border-slate-200 rounded-md p-5 space-y-3 text-left">
 <div className="flex gap-0.5">
 {[1, 2, 3, 4, 5].map((s) => (
 <Star
 key={s}
 ...
 <span
 key={tag}
 className="px-2.5 py-1 bg-blue-100 text-blue-700 text-[10px] font-bold rounded-lg"
 >
 {tag}
 </span>
 ))}
 ...
 <div className="w-8 h-8 rounded bg-blue-500 flex items-center justify-center text-white font-black text-xs border border-slate-200">
 ...
 <div className="bg-[#e5ddd5] #1a2436] rounded-md p-3">
 {/* Fake WA chat bubble */}
 <div className="bg-white #202c33] rounded-md rounded-tl-none px-3.5 py-2.5 max-w-full shadow-sm">
 <pre
 className="text-[11px] text-slate-800 leading-relaxed whitespace-pre-wrap font-sans break-words"
 ...
 {/* CTA notice */}
 <div className="flex items-start gap-2.5 p-3 bg-emerald-50 border border-emerald-200 rounded">
 ...
 className="flex-[2] py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold text-sm rounded border border-slate-200 cursor-pointer flex items-center justify-center gap-2 transition-colors"
 >
 <MessageCircle className="w-4 h-4" />
 Send via WhatsApp
 </button>
 ...
 <section className="bg-slate-100/50 border-y border-slate-200 py-16">
 ...
 <div className="inline-flex items-center gap-2 px-6 py-3.5 bg-amber-400 border border-slate-200 rounded cursor-pointer hover:bg-amber-300 transition-colors font-black text-slate-950 text-sm"
 ...
 {[
 { icon: Star, value:"4.9/5", label:"Average Rating", bg:"bg-amber-50 text-amber-600" },
 { icon: Trophy, value:"250+", label:"Reviews Received", bg:"bg-blue-50 text-blue-600" },
 { icon: CheckCircle2, value:"98%", label:"Positive Reviews", bg:"bg-emerald-50 text-emerald-600" },
 { icon: MessageCircle, value:"24h", label:"Response Time", bg:"bg-purple-50 text-purple-600" },
 ].map(({ icon: Icon, value, label, bg }, idx) => (
 <div
 key={idx}
 className={`${bg} border border-slate-200 rounded-md p-5 flex flex-col items-center text-center gap-2`}
 >
