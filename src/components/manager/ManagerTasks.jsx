...
export default function ManagerTasks() {
 ...
 <th className="p-4 font-semibold text-sm text-slate-600">Project</th>
 <th className="p-4 font-semibold text-sm text-slate-600">Assigned To</th>
 ...
 <td className="p-4 text-sm font-medium text-slate-900">{t.title}</td>
 <td className="p-4 text-sm text-slate-600">{t.project?.title ||"N/A"}</td>
 <td className="p-4 text-sm text-slate-600">{t.assignedTo?.fullName ||"Unassigned"}</td>
 <td className="p-4 text-sm text-slate-600">
 <span className="px-3 py-1 rounded text-xs font-medium bg-slate-100">{t.status}</span>
 </td>
 <td className="p-4 text-sm">
 <div className="flex gap-2">
 <button onClick={() => openEditModal(t)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg">
 ...
 <select required value={formData.project} onChange={e => setFormData({...formData, project: e.target.value})} className="w-full p-2 border rounded">
 ...
 <select value={formData.assignedTo} onChange={e => setFormData({...formData, assignedTo: e.target.value})} className="w-full p-2 border rounded">
 ...
 <select value={formData.priority} onChange={e => setFormData({...formData, priority: e.target.value})} className="w-full p-2 border rounded">
 ...
 <input type="date" value={formData.dueDate} onChange={e => setFormData({...formData, dueDate: e.target.value})} className="w-full p-2 border rounded" />
 </div>
 
 <div className="flex justify-end gap-3 pt-4">
 <button type="button" onClick={() => { setIsAddModalOpen(false); setIsEditModalOpen(false); }} className="px-4 py-2 bg-slate-200 rounded">Cancel</button>
