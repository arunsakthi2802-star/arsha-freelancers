...
export default function AIProjectAdvisor() {
 ...
 <div className={`bg-white border-2 border-slate-900 rounded-md p-5 sm:p-6 flex flex-col justify-between shadow-sm hover:shadow-md transition-shadow hover:shadow-md transition-shadow hover:shadow-sm hover:shadow-md transition-shadow hover:shadow-md transition-shadow hover:-translate-y-1 transition-all cursor-default group ${layout === 'horizontal' ? 'min-w-[240px] w-[240px] sm:min-w-[280px] sm:w-[280px] flex-shrink-0' : 'w-full'}`}>
 <div className="space-y-4">
 <div className="flex justify-between items-start">
 <span className="text-[10px] font-black uppercase px-2 py-1 bg-white border-t-2 border-blue-500 text-slate-900 rounded-lg border border-slate-900">
 {project.category}
 </span>
 <span className="flex items-center gap-1 text-[11px] font-bold text-slate-600 bg-slate-100 px-2 py-1 rounded-lg border border-slate-200">
 ...
 {layout !== 'horizontal' && (
 <div className="flex flex-col gap-2 bg-slate-50 p-3 rounded border border-slate-100">
 {project.summary && project.summary.map((line, i) => (
 <p key={i} className="text-xs text-slate-600 font-medium leading-relaxed flex items-start gap-2">
 ...
 <span key={tech} className="text-[9px] font-bold px-2 py-1 bg-white text-slate-700 border border-slate-200 rounded-md">
 {tech}
 </span>
 ))}
 {project.technology.length > 3 && (
 <span className="text-[9px] font-bold px-2 py-1 bg-slate-100 text-slate-500 rounded-md border border-slate-200">
 +{project.technology.length - 3}
 </span>
 )}
 </div>
 ...
 <span className="text-xs font-bold text-slate-800">{project.duration}</span>
 </div>
 ...
 className="mt-6 w-full py-3 bg-blue-600 hover:bg-blue-500 text-white font-black text-xs rounded flex items-center justify-center gap-2 border border-blue-800 shadow-sm hover:shadow-md transition-shadow hover:shadow-md transition-shadow active:translate-y-[2px] active:shadow-none transition-all cursor-pointer"
 >
 ...
 className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded shadow-sm hover:shadow-md transition-shadow hover:shadow-md transition-shadow font-bold text-sm cursor-pointer"
 >
 ...
 {!isChatClosed && (
 <div className={`bg-slate-900 rounded-md p-4 sm:p-8 relative overflow-hidden border-2 border-slate-800 shadow-xl flex flex-col transition-all duration-300 ${isChatMinimized ? 'h-24 sm:h-28' : 'h-[calc(100vh-120px)] min-h-[600px] max-h-[900px]'}`}>
 {/* Background elements */}
 ...
 {/* Avatar */}
 <div className={`w-8 h-8 rounded flex items-center justify-center flex-shrink-0 ${msg.sender === 'user' ? 'bg-white border-t-2 border-blue-500 text-slate-900' : 'bg-blue-600 text-white'}`}>
 {msg.sender === 'user' ? <User className="w-4 h-4" /> : <Bot className="w-4 h-4" />}
 </div>
 
 {/* Message Bubble */}
 <div className="flex flex-col gap-2">
 <div className={`px-4 py-3 rounded-md text-sm font-medium ${msg.sender === 'user' ? 'bg-white border-t-2 border-blue-500 text-slate-900 rounded-tr-none' : 'bg-slate-800 text-white border border-slate-700 rounded-tl-none'}`}>
 {msg.text}
 </div>
 ...
 <div className="w-8 h-8 rounded flex items-center justify-center flex-shrink-0 bg-blue-600 text-white">
 <Bot className="w-4 h-4" />
 </div>
 <div className="px-5 py-4 rounded-md bg-slate-800 text-white border border-slate-700 rounded-tl-none flex items-center gap-1.5">
 <span className="w-1.5 h-1.5 bg-blue-400 rounded animate-bounce"></span>
 <span className="w-1.5 h-1.5 bg-blue-400 rounded animate-bounce delay-75"></span>
 <span className="w-1.5 h-1.5 bg-blue-400 rounded animate-bounce delay-150"></span>
 </div>
 </div>
 </div>
 )}
 ...
 className="w-full bg-slate-800/80 backdrop-blur text-white pl-4 sm:pl-6 pr-14 sm:pr-16 py-3 sm:py-4 rounded-md border border-slate-700 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 font-bold shadow-xl transition-all text-xs sm:text-sm"
 />
 <button 
 type="submit"
 disabled={!chatInput.trim() || isTyping}
 className="absolute right-1.5 sm:right-2 top-1.5 sm:top-2 p-2 sm:p-2.5 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 text-white rounded transition-colors cursor-pointer"
 >
 ...
 {/* Advanced Filters */}
 <div className="bg-white p-6 rounded-md border-2 border-slate-900 shadow-sm hover:shadow-md transition-shadow hover:shadow-md transition-shadow">
 <div className="flex items-center gap-2 mb-4 text-slate-900">
 <Filter className="w-5 h-5 text-blue-600" />
 <h3 className="font-black text-sm uppercase tracking-wider">Manual Filters</h3>
 </div>
 <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
 <select value={selectedDept} onChange={(e) => setSelectedDept(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded text-xs font-bold text-slate-800 focus:outline-none cursor-pointer">
 {allDepts.map(d => <option key={d} value={d}>{d === 'All' ? 'All Departments' : d}</option>)}
 </select>
 <select value={selectedTech} onChange={(e) => setSelectedTech(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded text-xs font-bold text-slate-800 focus:outline-none cursor-pointer">
 {allTechs.map(t => <option key={t} value={t}>{t === 'All' ? 'All Technologies' : t}</option>)}
 </select>
 <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded text-xs font-bold text-slate-800 focus:outline-none cursor-pointer">
 {allCategories.map(c => <option key={c} value={c}>{c === 'All' ? 'All Categories' : c}</option>)}
 </select>
 <select value={selectedDifficulty} onChange={(e) => setSelectedDifficulty(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded text-xs font-bold text-slate-800 focus:outline-none cursor-pointer">
 ...
 <h2 className="text-xl font-black text-slate-900">Filtered Results ({filteredProjects.length})</h2>
 <button 
 onClick={() => { setSelectedDept('All'); setSelectedTech('All'); setSelectedCategory('All'); setSelectedDifficulty('All'); }}
 className="text-xs font-bold text-slate-500 hover:text-slate-900 px-3 py-1.5 bg-slate-100 rounded-lg transition-colors cursor-pointer"
 >
 Clear Filters
 </button>
 </div>
 ...
 <h2 className="text-2xl font-black text-slate-900 tracking-tight">Trending Projects</h2>
 </div>
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
 ...
 <section className="space-y-6">
 <div className="flex items-center gap-3">
 <div className="p-2 bg-amber-100 rounded text-amber-600 border border-amber-200">
 <Star className="w-6 h-6" />
 </div>
 <h2 className="text-2xl font-black text-slate-900 tracking-tight">Recommended Projects</h2>
 </div>
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
 ...
 <section className="space-y-6">
 <div className="flex items-center gap-3">
 <div className="p-2 bg-blue-100 rounded text-blue-600 border border-blue-200">
 <Clock className="w-6 h-6" />
 </div>
 <h2 className="text-2xl font-black text-slate-900 tracking-tight">Newly Added</h2>
 </div>
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
 ...
 <section className="space-y-6">
 <div className="flex items-center gap-3">
 <div className="p-2 bg-emerald-100 rounded text-emerald-600 border border-emerald-200">
 <BookOpen className="w-6 h-6" />
 </div>
 <h2 className="text-2xl font-black text-slate-900 tracking-tight">Beginner Friendly</h2>
 </div>
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
 ...
 <section className="space-y-6">
 <div className="flex items-center gap-3">
 <div className="p-2 bg-purple-100 rounded text-purple-600 border border-purple-200">
 <Microscope className="w-6 h-6" />
 </div>
 <h2 className="text-2xl font-black text-slate-900 tracking-tight">Research Projects (Publication Ready)</h2>
 </div>
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
 ...
 <section className="space-y-6">
 <div className="flex items-center gap-3">
 <div className="p-2 bg-slate-200 rounded text-slate-600 border border-slate-300">
 <BookOpen className="w-6 h-6" />
 </div>
 ...
 className="p-4 bg-white border-2 border-slate-200 rounded-md hover:border-blue-500 hover:shadow-sm hover:shadow-md transition-shadow hover:shadow-md transition-shadow transition-all text-left flex flex-col items-center justify-center text-center gap-2 cursor-pointer"
 >
